...
// ---- Profile update (current user only) ----
if ($action === 'profile_update') {
    // Current user is already authenticated via requireAuth()
    $newDisplay = trim((string)($input['display_name'] ?? ''));
    ...
    backupBeforeWrite('users.json');
    writeJson('users.json', $users);
    jsonResponse(['ok' => true, 'display_name' => $newDisplay]);
}
...
if ($action === 'change_password') {
    ...
    if (!$currentPassword) jsonResponse(['error' => 'يرجى إدخال كلمة المرور الحالية'], 400);
    if (!$newPassword) jsonResponse(['error' => 'يرجى إدخال كلمة المرور الجديدة'], 400);
    if (strlen($newPassword) < 8) jsonResponse(['error' => 'كلمة المرور يجب أن تكون 8 أحرف على الأقل'], 400);
    ...
    if (!$found) jsonResponse(['error' => 'المستخدم غير موجود'], 404);
    if (!$passwordMatch) jsonResponse(['error' => 'كلمة المرور الحالية غير صحيحة'], 401);
